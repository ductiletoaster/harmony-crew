...
A **generic agent foundation** — 7 role agents + a library of generic skills (conventions, tool-usage, runtime) you drop into your projects — dual-published for two runtimes:
...
> A **personal toolkit**, not an OSS package: the foundation carries the patterns, the **tool-usage skills** for the stack you run across your projects, and the agent→tool routing. Each consuming project supplies its own specifics — which tools, where, plus a few facts — in its overlay (`.pi/`, `.claude/`) and the agent's working context. No one project's *values* are baked in; the overlay still shadows the foundation when names collide.

## What's in the box

**7 role agents** — operational shapes, not behavioural knowledge. Capability is added as a skill, not a new agent.
...
| `researcher` | Option analysis → a structured note (via the project's memory module). |
...
**Skills** (`skills/<name>/SKILL.md`) come in three kinds: generic **pattern** skills (stack conventions — python/terraform/ansible/k8s; process — pr-review/intake/plan/seam-detection); **tool-usage** skills (`tool-<x>/` — argocd, litellm, external-secrets, vault-substrate, comfyui, coder, searxng); and the **routing contract** (`agent-routing`). A consuming project loads the tool-usage skills it needs and supplies their values through its own overlay/context.
